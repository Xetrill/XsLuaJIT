...
/// above this index to open space. Cannot be called with a
...
LUA_API const void *lua_topointer (lua_State *L, int idx)
{
    ...
    case LUA_TBOOLEAN:       return &bvalue(o);
    case LUA_TNUMBER:        return &nvalue(o);
    case LUA_TUPVAL:         return uvalue(o);
...
LUA_API void lua_rawsetr (lua_State *L, int idx)
{
    StkId t;
    lua_lock(L);
    api_checknelems(L, 2);
    t = index2adr(L, idx);
    api_check(L, ttistable(t));
    setobj2t(L, luaH_set(L, hvalue(t), L->top-1), L->top-2);
    luaC_barriert(L, hvalue(t), L->top-2);
    L->top -= 2;
    lua_unlock(L);
}
...
/// 
/// If there are no more elements in the table, then lua_next returns 0
/// (and pushes nothing).
/// 
...
/// key, unless you know that the key is actually a string.
/// Recall that lua_tolstring changes the value at the given index; this
/// confuses the next call to lua_next. 
