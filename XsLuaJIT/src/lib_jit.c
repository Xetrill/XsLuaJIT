...
LUALIB_API int luaopen_jit(lua_State *L)
{
    ...
    if (lxs_canextend(L))
    {
        ...
        retval += luaopen_marshal(L);
#endif
#if LUAXS_ADDLIB_BUFFER
        retval += luaopen_buffer(L);
#endif
#if LUAXS_ADDLIB_GAME
        retval += luaopen_game(L);
#endif
#if LUAXS_ADDLIB_CONTAINER
        retval += luaopen_container(L);
#endif
#if LUAXS_ADDLIB_MEMORY
        retval += luaopen_memory(L);
#endif
    ...
    if (luaJIT_setmode(L, 0, LUAJIT_MODE_ENGINE | LUAJIT_MODE_ON) <= 0)
    {
        ...
        return lxs_error(L, "JIT engine init failed (%d)",
