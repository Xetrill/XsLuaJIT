...
#if LUAXS_CORE_TERSE
#  define SCRIPTS_DIR (LUA_DIRSEP "scripts" LUA_DIRSEP)
#endif
...
#if LUAXS_CORE_TERSE
            mark = CONST_CAST(char*, strstr(source, SCRIPTS_DIR));
            if (mark/* && strlen(mark) < bufflen*/)
            {
                strncpy(out, mark + sizeof(SCRIPTS_DIR) - 1, bufflen);
                return;
            }
#endif
